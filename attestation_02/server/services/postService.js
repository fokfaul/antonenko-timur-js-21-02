...
class PostService {
  getPostList(req, res) {
    ...
    doGetRequest(POST_URL, req.query).then((apiData) => {
        if ('error' in apiData)
        {
            logger.info(format(messages.GET_POST_LIST_ERROR, 400, apiData))
  ...
  getPostById(req, res) {
    ...
    doGetRequest(`${POST_URL}/${req.params.id}`).then((apiData) => {
        if ('error' in apiData)
        {
            logger.info(format(messages.GET_POST_ERROR, 400, apiData))
            res.status(400).send({"error": apiError(apiData['error'])})
        }
        else
        {
  ...
  getPostComment(req, res) {
    ...
    doGetRequest(`${POST_URL}/${req.params.id}/comment`, req.body).then((apiData) => {
        if ('error' in apiData)
        {
            logger.info(format(messages.POST_COMMENT_ERROR, 400, apiData))
            res.status(400).send({"error": apiError(apiData['error'])})
        }
        else
        {
